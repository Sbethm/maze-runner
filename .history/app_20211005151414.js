...
const iteratingMazeWalls = (row, column) => {
    ...
    const randomNeighbors = [];

    const createRandomList = (neighbors) => {
        for(let i = 0; randomNeighbors.length = 4 ; i++) {
            const listItem = neighbors[Math.floor(Math.random() * cells)];
            console.log(listItem);
            if(randomNeighbors.includes(listItem)) {
                return;
            } else {
                randomNeighbors.push(listItem); 
            }
        }
    }

    createRandomList(neighbors);
    console.log(randomNeighbors);
    //For each neighbor...
